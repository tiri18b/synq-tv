...
export default function TV() {
  ...
  const expiredUrgents = posts.filter((p) => {
    ...
    return (
      <div className="urgent-fullscreen">
        ...
        <div className="urgent-box">
          ...
          <small>ההודעה תעבור להתראה רגילה לאחר 10 דקות</small>
        </div>
  ...
  return (
    <div className="tv-screen">
      {selectedAlert && (
        <div className="alert-modal" tabIndex="0" onClick={closeAlert} onKeyDown={(e) => e.key === "Enter" && closeAlert()}>
          <div className="alert-modal-box">
            <div className="urgent-label">⚠️ התראה חשובה</div>
            {selectedAlert.image_url && <img className="urgent-image" src={selectedAlert.image_url} alt={selectedAlert.title} />}
      ...
      <main className="tv-grid">
        ...
        <section className="side-card">
          ...
          {expiredUrgents.length === 0 ? (
            <p>אין התראות פעילות.</p>
          ) : (
            expiredUrgents.slice(0, 3).map((alert) => (
              <button
                key={alert.id}
                className="tv-alert-button"
                type="button"
                onClick={() => openAlert(alert)}
                onKeyDown={(e) => e.key === "Enter" && openAlert(alert)}
              >
                <span>⚠️</span>
...
console.log("TV updated with 10 minute urgent behavior");
