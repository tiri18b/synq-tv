...
export default function TV() {
  ...
  const alertUrgents = posts.filter((p) => {
    ...
    return (
      ...
      >
        ...
        <div className="urgent-box">
          ...
          <small>לחץ OK / Enter כדי להעביר את ההודעה לצד כהתראה</small>
        </div>
    ...
    <div className="tv-screen">
      ...
      <main className="tv-grid">
        ...
        <section className="side-card">
          ...
          {alertUrgents.length === 0 ? (
            <p>אין התראות פעילות.</p>
          ) : (
            alertUrgents.slice(0, 3).map((alert) => (
              <button
                key={alert.id}
...
console.log("TV rebuilt with dismiss urgent feature");
